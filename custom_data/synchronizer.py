...
def export_audio_with_click(proc_res, path_audio, path_output, sr=44100):
    ...
    # click audio
    y_beat = (
        librosa.clicks(times=times_beat, sr=sr, click_freq=1200, click_duration=0.5)
        * 0.6
    )
    y_downbeat = librosa.clicks(
        times=times_downbeat, sr=sr, click_freq=600, click_duration=0.5
    )
...
def analyze(
    path_midi_input, path_audio_input, path_midi_output, path_audio_output=None
):
...
if __name__ == "__main__":
    ...
    # list files
    midifiles = traverse_dir(path_indir, is_ext=False, is_pure=True, is_sort=True)
